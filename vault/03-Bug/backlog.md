...
- Stato: one-event Databento micro-probes passati in [[Report-Databento-Provider-Evaluation-Preflight-2026-05-17]] dopo autorizzazione user a usare fino a `125 USD` free credits. Primo tentativo falli' con `BentoClientError / 401 auth_authentication_failed`; follow-up: script hardenato con `--api-key-source auto|environment|env-file`; dopo correzione `.env`, metadata smoke-test passa. Probe `EQUS.MINI / trades / FSR / 2024-03-20T14:30..14:35 / limit=10` restituisce `records=10`; dataset diagnostics passa: `trades` disponibile, `FSR -> instrument_id 6354`, `record_count=10`, `estimated_cost_usd=0.000002682209`, OHLCV schemas disponibili. Probe `ohlcv-1m` sullo stesso evento restituisce `records=5`. Raw payload non trattenuto; validator pass `failed=0`, `passed=21`. Provider data solo parzialmente valutati su un evento.
