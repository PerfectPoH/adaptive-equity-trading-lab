...
- Stato: minimal full-panel readiness raggiunta in [[Report-Databento-Provider-Evaluation-Preflight-2026-05-17]]. Dopo preflight, auth fix, metadata diagnostics e micro-probe FSR, sono stati eseguiti tiny OHLCV probes su DPE-001..DPE-010 senza raw retention. Tutti gli eventi hanno `provider_symbol_resolves=yes`, `event_window_available=yes`, `raw_ohlcv_available=yes`; record summary: TUP=2, MULN=5, CNGL=4, ABAT=5, WEYS=8, FSR=10+5, PHUN=5, GH=6, ICU=2, DWAC/DJT=3+5. Verdict resta `caveat`: adjusted OHLCV, corporate actions, halt/suspension metadata, PIT universe e license/storage rights non sono ancora pienamente testati. Stato: `CONTROLLED_FULL_PANEL_EVALUATION_EXECUTED / DATABENTO_CAN_PROCEED_TO_NEXT_FULL_PANEL_STEP / PROVIDER_VERDICT=CAVEAT_NOT_FINAL_PASS`. Artifact derivato `full_panel_derived_evaluation.csv` creato; requirement table aggiornata con pass/partial/caveat.
...
### RESEARCH-072 - Provider sensitivity coverage-aware expansion
...
### RESEARCH-080 - Research run gate validator
...
Update: Intrinio follow-up answers resolved informational blockers for US small-cap coverage, adjusted/unadjusted availability, delisted symbols, suggested endpoints (`stock_prices_v2`, `security_historical_data_v2`), rate limit (`2000 calls/min`), and internal research/backtesting validation use. Gate status advanced to `SPEC_ONLY_INTRINIO_TRIAL_INFO_RESOLVED_NOT_QUERIED`; validator now passes 42/42. Remaining blockers: rotate/replace previously exposed key, separate one-call probe approval, output directory, trial ledger entry.

Update: Intrinio key rotation blocker resolved. User replaced the key and stored it in `.env` as `INTRINIO_API_KEY`; presence-only env-file credential preflight passed without network call or secret disclosure. Gate status is now `SPEC_ONLY_INTRINIO_CREDENTIAL_READY_NOT_QUERIED`. Remaining blockers before any API call: separate one-call probe approval, output directory, trial ledger entry.
