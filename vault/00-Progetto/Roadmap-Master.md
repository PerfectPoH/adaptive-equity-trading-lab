...
- [x] Documentare Lessons Learned data-quality: lavoro small-cap 2026-05-09..2026-05-14 riclassificato come stress test metodologico/infrastrutturale, non evidenza di edge.
- [x] Eseguire scaffolding check tecnico su universo fixed large-cap/ETF control, senza tuning delle soglie strategiche: `TECHNICAL_PASS`, artefatti completi e manifest `extras.purpose=nctrl_scaffolding_check`.
- [ ] Redigere preregistrazione `TRIAL-NCTRL-001` come property-based negative control; non eseguire prima del documento singolo.

Gate metodologico corrente: run manifest, stress test ex-outlier, smoke ampia, cash starvation diagnostics, setup disentangler passivo, feature diagnostics per setup, breakout-only ablation, feature filter ablation, open-to-close sensitivity, temporal split validation, multi-year validation e passive regime diagnostics sono implementati. Dopo RankEx, il prossimo collo di bottiglia primario e' data-quality/methodology: data-quality audit result e Lessons Learned sono completati; non pre-registrare nuovi trial small-cap finche' non cambia dataset/provider. Scaffolding check tecnico fixed large-cap/ETF completato con `TECHNICAL_PASS`; prossimo lavoro ammesso: preregistrazione singola `TRIAL-NCTRL-001` come property-based negative control, non esecuzione. La smoke ampia ha prodotto `portfolio_return=-22.16%`, score profile non monotono e missed opportunities mediane negative.
