...
GapRev intraday data-contract gate added. Artifact: `experiments/provider_aware_research/gap_down_reversion_intraday_data_contract_gate_20260521/`; report: [[Report-GapRev-Intraday-Data-Contract-Gate-2026-05-21]]. Validator `src.experiments.gaprev_intraday_data_contract_validator` passes `35/35`; targeted tests pass `5/5`. The gate requires intraday bars, timezone/session calendar, prior close/open mapping, spread or quote proxy, slippage/impact/commission model and raw payload retention blocked by default. No provider is selected and no query/download/extractor/backtest is authorized.

PEAD SUE entitlement pack added. Artifact: `experiments/provider_aware_research/pead_sue_entitlement_pack_20260521/`; report: [[Report-PEAD-SUE-Entitlement-Pack-2026-05-21]]. Validator `src.experiments.pead_sue_entitlement_pack` passes `19/19`; targeted tests pass `5/5`. The pack turns the `BLOCKED_PIT_SUE_PROVIDER_UNAVAILABLE` state into provider questions, blocking acceptance criteria and a one-call CRMD SUE probe template. It authorizes no provider query, no raw payload retention, no PEAD backtest, no paper/live and no promotion.

Trade governance runtime added. Module: `src.execution.trade_governance`; artifact: `experiments/provider_aware_research/trade_governance_runtime_spec_20260521/`; report: [[Report-Trade-Governance-Runtime-2026-05-21]]. It classifies trades into `GOOD_WIN`, `GOOD_LOSS`, `BAD_WIN`, `BAD_LOSS`, excludes `BAD_WIN` from governed edge metrics, and enforces same-symbol cooldown after stop-loss exits. This is broker-free infrastructure only: no provider query, broker connection, backtest, paper/live or promotion.

Alpha candidate factory added. Module: `src.experiments.alpha_candidate_factory`; artifact: `experiments/provider_aware_research/alpha_candidate_factory_20260521/`; report: [[Report-Alpha-Candidate-Factory-2026-05-21]]. It ranks 12 falsifiable candidates across PEAD/SUE, dollar bars, Amihud, quality-filtered small-cap momentum, biotech catalysts, SEC event filters, provider fragility and governance integration. Validator passes `16/16`; targeted tests pass `5/5`. Top candidate is `ALPHA-009`: outlier-resistant diagnostics on existing artifacts. No provider query, backtest, paper/live or promotion.

Outlier-resistant diagnostic harness added for `ALPHA-009`. Module: `src.experiments.outlier_resistant_diagnostic_harness`; artifact: `experiments/provider_aware_research/outlier_resistant_diagnostic_20260521/`; report: [[Report-Outlier-Resistant-Diagnostic-Harness-2026-05-21]]. It ranks four existing trade-log artifacts by trade count, total PnL, median PnL, top-winner contribution, PnL excluding top three winners and sign-flip risk. Result: no robust survivor. Top score is provider-aware bounded replay (`25`) but total PnL is negative. XMOM remains worst (`5`) because median trade is negative and PnL flips after removing top three. No provider query, no new backtest, no paper/live or promotion.

Dollar-bar diagnostic added for `ALPHA-003`. Module: `src.experiments.dollar_bar_diagnostic`; artifact: `experiments/provider_aware_research/dollar_bar_diagnostic_20260521/`; report: [[Report-Dollar-Bar-Diagnostic-2026-05-21]]. It transforms 12 stored Databento intraday `ohlcv-1m` files into dollar bars using a non-optimized bucket (`total_dollar_volume / time_bar_count`) and compares skew, Pearson kurtosis and 3-sigma outlier rate. Result: 11/12 files show distribution-stability improvement, median stability delta `19.806168`. This is not alpha and authorizes no strategy test; next safe step is a separate preregistered data-transform validator.

Dollar-bar transform preregistration added. Module: `src.experiments.dollarbar_transform_preregistration_validator`; artifact: `experiments/provider_aware_research/dollarbar_transform_preregistration_20260521/`; report: [[Report-DollarBar-Transform-Preregistration-2026-05-21]]. It freezes 12 existing intraday bar files, defines baseline `static_average_dollar_bucket` and candidate `rolling_ema_dollar_bucket`, allows only distribution metrics (`absolute_skew`, Pearson kurtosis, 3-sigma outlier rate, bar-count stability, median stability delta, coverage), and forbids PnL, win rate, signal return, trade outcome, Sharpe, profit factor and strategy return. Validator passes `24/24`; no execution or strategy test.

Dollar-bar transform validation completed. Module: `src.experiments.dollarbar_transform_validator`; artifact: `experiments/provider_aware_research/dollarbar_transform_validation_20260521/`; report: [[Report-DollarBar-Transform-Validation-2026-05-21]]. Static average-dollar bars were compared against predeclared rolling EMA dollar bars (`span=20`) using distribution-only metrics. Validation artifacts pass `14/14`, but the transform verdict is `DOLLARBAR_TRANSFORM_REJECTED_FOR_DATA_LAYER`: only `1/12` files improved, coverage rate `0.083333`, median stability score delta `-3.556627`. No PnL, signal, backtest, provider query, paper/live or promotion occurred.

DollarBar MicroRev five-step protocol completed. Module: `src.experiments.dollarbar_microrev_trial`; artifacts: `static_dollarbar_data_layer_contract_20260521/`, `dollarbar_ema_rejection_20260521/`, `dollarbar_microrev_preregistration_20260521/`, and `execution_outputs/DOLLARBAR-MICROREV-RUN-001/`; report: [[Report-DollarBar-MicroRev-Trial-2026-05-21]]. Static average-dollar bars are canonical for this branch, EMA remains blocked, and `TRIAL-DOLLARBAR-MICROREV-001` is archived in current form. Result on 12 existing intraday files: 140 trades, gross return sum `-0.8469489896`, net return sum after 500 bps `-7.8469489896`, positive net trade rate `0.0`. No provider query, parameter sweep, paper/live, or promotion occurred.

Amihud liquidity toxicity diagnostic completed. Module: `src.experiments.amihud_liquidity_toxicity_diagnostic`; artifact: `experiments/provider_aware_research/amihud_liquidity_toxicity_20260522/`; report: [[Report-Amihud-Liquidity-Toxicity-Diagnostic-2026-05-22]]. It computes 20-day pre-entry Amihud illiquidity for the 11 XMOM trades using existing Databento daily prices only. Verdict: `LIQUIDITY_TOXICITY_DIAGNOSTIC_ARCHIVE_CURRENT_FORM`; high-ILLIQ loser rate `0.333333`, low-ILLIQ loser rate `0.8`, separation `-0.466667`, high-ILLIQ median PnL `37896.174002`, low-ILLIQ median PnL `-9846.473523`. The liquidity-toxicity filter is not supported. No provider query, new backtest, sweep, paper/live, or promotion occurred.

SEC 8-K event timing diagnostic completed. Module: `src.experiments.sec_8k_event_timing_diagnostic`; artifact: `experiments/provider_aware_research/sec_8k_event_timing_diagnostic_20260522/`; report: [[Report-SEC-8K-Event-Timing-Diagnostic-2026-05-22]]. It compares existing CRMD SEC 8-K Item 2.02 reaction-session days against non-event days using existing Databento daily prices only. Event windows are distinctive: event median absolute return `0.202955665` vs control `0.0244988864`, event median volume ratio `3.75641665` vs control `1.00298051`. Verdict remains `SEC_8K_TIMING_DIAGNOSTIC_ARCHIVE_CURRENT_FORM` because only 7 event days are covered, below the sample gate. No provider query, market-data download, backtest, sweep, paper/live, or promotion occurred.

SEC 8-K multi-symbol event timing diagnostic completed. Module: `src.experiments.sec_8k_multisymbol_event_timing_diagnostic`; artifact: `experiments/provider_aware_research/sec_8k_multisymbol_event_timing_diagnostic_20260522/`; report: [[Report-SEC-8K-Multisymbol-Event-Timing-Diagnostic-2026-05-22]]. SEC submissions were queried for the five issuer symbols already present in the XMOM Databento price panel (`AEHR`, `ARRY`, `CABA`, `CRMD`, `IOVA`); raw SEC payloads were not retained and no market data was downloaded. Result: `SEC_8K_MULTISYMBOL_TIMING_CANDIDATE_ONLY`, 87 event days covered by prices, 4823 controls, event median abs return `0.0724637681` vs control `0.0299477103`, event median volume ratio `3.03102063` vs control `0.98366013`. This validates SEC Item 2.02 as a high-volatility/high-volume regime candidate, but it remains blocked for trading because there is no ex-ante direction or surprise source.

SEC8K/XMOM overlap diagnostic completed. Module: `src.experiments.sec_8k_xmom_overlap_diagnostic`; artifact: `experiments/provider_aware_research/sec_8k_xmom_overlap_diagnostic_20260522/`; report: [[Report-SEC8K-XMOM-Overlap-Diagnostic-2026-05-22]]. It uses only existing XMOM trade-log artifacts, existing Databento prices, and the existing SEC Item 2.02 event panel; no provider query, new backtest, parameter sweep, paper/live, or promotion occurred. Result: `SEC8K_XMOM_OVERLAP_SUPPORTS_CATALYST_EXPLANATION`, 11 trades, +/-5 trading-day fixed window, 8 overlaps, overlap rate `0.727273`, winner overlap rate `1.0`, loser overlap rate `0.5`, top3 winner overlap rate `1.0`, overlap median PnL `34574.641169`, outside median PnL `-7703.904097`. Blocker remains `trade_count_below_30`; interpretation is historical catalyst exposure only and must not be used as a trading filter without a separate preregistered direction-source study.

SEC8K Direction Tape Oracle preregistration completed. Validator: `src.experiments.sec8k_direction_tape_oracle_preregistration_validator`; artifact: `experiments/provider_aware_research/sec8k_direction_tape_oracle_preregistration_20260522/`; report: [[Report-SEC8K-Direction-Tape-Oracle-Preregistration-2026-05-22]]. Trial `TRIAL-SEC8K-DIRECTION-001` is spec-only and long-only positive-oracle. Frozen oracle: first RTH tape window `09:30-09:45 America/New_York`, hypothetical entry `09:46`, same-day flat by `15:55`, volume ratio threshold `3.0`, cost model `500` bps, DSR threshold `0.95`, minimum trade count `30`. Validator passes `51/51`; targeted tests pass `5/5`. Execution, provider query, intraday download, backtest, sweep, paper/live and promotion remain blocked. Next safe step is a separate intraday data-contract gate for SEC8K Tape Oracle.

SEC8K Tape Oracle intraday data-contract gate completed. Validator: `src.experiments.sec8k_tape_oracle_intraday_data_contract_validator`; artifact: `experiments/provider_aware_research/sec8k_tape_oracle_intraday_data_contract_gate_20260522/`; report: [[Report-SEC8K-Tape-Oracle-Intraday-Data-Contract-2026-05-22]]. The gate requires existing SEC Item 2.02 reaction-session panel, BMO/AMC-only eligibility, 1m-or-finer RTH bars, America/New_York timestamps, fixed oracle `09:30-09:45`, hypothetical entry `09:46`, flat `15:55`, same-symbol first-15m control volume baseline, spread/slippage/impact/commission proxies, and 500 bps round-trip cost model. Validator passes `51/51`; targeted tests pass `5/5`. Provider selection, provider query, intraday download, extractor, oracle signal computation, backtest, sweep, paper/live, promotion and short selling remain blocked. Next safe step is provider-selection and bounded approval for intraday RTH coverage.

SEC8K Tape Oracle existing-intraday bounded backtest completed. Module: `src.experiments.sec8k_tape_oracle_existing_intraday_backtest`; artifact: `experiments/provider_aware_research/execution_outputs/SEC8K-TAPE-ORACLE-EXISTING-INTRADAY-BACKTEST-001/`; report: [[Report-SEC8K-Tape-Oracle-Existing-Intraday-Backtest-2026-05-22]]. It used existing intraday artifacts only and found one SEC8K-matching event (`CRMD`, 2024-03-12). That event was purged because the fixed oracle/entry/exit windows were incomplete under the strict contract, so positive oracle trade count is `0`, gross/net return sums are `0`, and decision is `SEC8K_TAPE_ORACLE_ARCHIVE_CURRENT_EXISTING_DATA_FORM`. No provider query, market-data download, parameter sweep, paper/live or promotion occurred. Next safe step, if continuing this branch, is a separate bounded intraday provider approval to expand sample size.

SEC8K Tape Oracle Databento mini-panel backtest completed. Module: `src.experiments.sec8k_tape_oracle_databento_mini_panel`; data artifact: `experiments/provider_aware_research/data_inputs/sec8k_tape_oracle_databento_mini_panel_20260522/`; execution artifact: `experiments/provider_aware_research/execution_outputs/SEC8K-TAPE-ORACLE-DATABENTO-MINI-PANEL-BACKTEST-001/`; report: [[Report-SEC8K-Tape-Oracle-Databento-Mini-Panel-Backtest-2026-05-22]]. Bounded Databento scope: 30 SEC Item 2.02 event queries, `XNAS.ITCH`, `ohlcv-1m`, raw payload retention disabled. Query result: 30 pass, 0 empty, 0 error. Backtest result: 30 evaluated events, 4 positive-oracle trades, gross return sum `0.0505549675`, net return sum after 500 bps `-0.1494450325`. Decision: `SEC8K_TAPE_ORACLE_MINI_PANEL_NO_PROMOTION_COST_AND_SAMPLE_BLOCKED`. No parameter sweep, paper/live or promotion occurred.
