...
REQUIRED_TICKERS = ["AABA", "AAC", "AACQ", "AACT", "AAGR"]


def validate_polygon_delisted_listing_date_probe_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-DELISTED-LISTING-DATE-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-DELISTED-LISTING-DATE-PROBE-001", str(manifest.get("trial_id")))
    _check(checks, "source_run_id", manifest.get("source_run_id") == "POLYGON-DELISTED-SURVIVORSHIP-AUDIT-001", str(manifest.get("source_run_id")))
    _check(checks, "status_pre_run", manifest.get("status") == "APPROVED_FIVE_DELISTED_TICKER_DETAILS_CALLS_NOT_EXECUTED", str(manifest.get("status")))
    ...
    _check(checks, "delisted_tickers_fixed", manifest.get("delisted_tickers") == REQUIRED_TICKERS, str(manifest.get("delisted_tickers")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_DELISTED_LISTING_DATE_PROBE_GATE_PASS" if failed == 0 else "POLYGON_DELISTED_LISTING_DATE_PROBE_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon delisted listing-date probe gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_delisted_listing_date_probe_gate(args.spec_dir)
