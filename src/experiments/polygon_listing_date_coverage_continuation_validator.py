...
REQUIRED_TICKERS = ["AAL", "AAME", "AAMI", "AAOI", "AAON"]


def validate_polygon_listing_date_coverage_continuation_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-LISTING-DATE-COVERAGE-CONTINUATION-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-LISTING-DATE-COVERAGE-CONTINUATION-001", str(manifest.get("trial_id")))
    _check(checks, "source_run_id", manifest.get("source_run_id") == "POLYGON-LISTING-DATE-COVERAGE-PROBE-001", str(manifest.get("source_run_id")))
    _check(checks, "status_pre_run", manifest.get("status") == "APPROVED_FIVE_TICKER_DETAILS_CONTINUATION_NOT_EXECUTED", str(manifest.get("status")))
    _check(checks, "provider_polygon", manifest.get("provider") == "Polygon/Massive", str(manifest.get("provider")))
    _check(checks, "endpoint_template_fixed", manifest.get("endpoint_template") == "https://api.polygon.io/v3/reference/tickers/{ticker}", str(manifest.get("endpoint_template")))
    _check(checks, "api_key_env", manifest.get("api_key_env_var") == "POLYGON_API_KEY", str(manifest.get("api_key_env_var")))
    _check(checks, "remaining_tickers_fixed", manifest.get("remaining_tickers") == REQUIRED_TICKERS, str(manifest.get("remaining_tickers")))
    _check(checks, "five_call_bound", manifest.get("expected_max_calls") == 5, str(manifest.get("expected_max_calls")))
    _check(checks, "rate_limit_delay", int(manifest.get("min_seconds_between_calls", 0)) >= 13, str(manifest.get("min_seconds_between_calls")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_LISTING_DATE_COVERAGE_CONTINUATION_GATE_PASS" if failed == 0 else "POLYGON_LISTING_DATE_COVERAGE_CONTINUATION_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon listing-date coverage continuation gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_listing_date_coverage_continuation_gate(args.spec_dir)
