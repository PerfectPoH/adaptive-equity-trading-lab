...
RUN_ID = "POLYGON-ACTIVE-UNIVERSE-SEED-001"
TRIAL_ID = "UNIVERSE-ACTIVE-SEED-001"
SPEC_DIR = Path("experiments/provider_aware_research/polygon_active_universe_seed_20260524")
OUTPUT_DIR = Path("experiments/provider_aware_research/execution_outputs/POLYGON-ACTIVE-UNIVERSE-SEED-001")
VAULT_REPORT = Path("vault/04-Documentazione/Reports/Report-Polygon-Active-Universe-Seed-001-2026-05-24.md")


def run_polygon_active_universe_seed_001(
    *,
    ...
    gate = validate_polygon_active_universe_seed_gate(spec_dir)
    ...
    manifest = json.loads((Path(spec_dir) / "seed_manifest.json").read_text(encoding="utf-8"))
    ...
    if not api_key:
        ...
        try:
            ...
            assessment = assess_polygon_active_universe_seed_payload(payload, manifest=manifest)
    ...
    seed_rows = list(assessment.get("active_common_stock_seed", []))
    _write_csv(output / "active_common_stock_seed.csv", _fieldnames(seed_rows), seed_rows)
    _write_json(output / "active_universe_seed_assessment.json", assessment)
    _write_json(output / "final_decision.json", decision)
    _write_vault_report(Path(vault_report), assessment, decision)
    return decision


def assess_polygon_active_universe_seed_payload(payload: dict[str, Any], manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    spec = manifest or {
        "required_fields": ["ticker", "primary_exchange", "type", "active"],
        "required_exchanges": ["XNAS", "XNYS", "XASE"],
        "required_security_type": "CS",
        "min_active_common_stock_seed_count": 300,
        "min_metadata_coverage": 0.95,
        "requires_survivorship_audit_before_backtest": True,
        "requires_liquidity_probe_before_backtest": True,
    }
    ...
    derived_seed = _derived_rows(seed_rows)
    blockers: list[str] = []
    if len(derived_seed) < int(spec["min_active_common_stock_seed_count"]):
        blockers.append("active_common_stock_seed_count_below_300")
    ...
    return {
        ...
        "active_common_stock_seed_count": len(derived_seed),
        "metadata_coverage": coverage,
        "required_exchanges": sorted(required_exchanges),
        "required_security_type": security_type,
        "passes_active_universe_seed": len(blockers) == 0,
        "blockers": blockers,
        "active_common_stock_seed": derived_seed,
        "provider_query_performed": True,
        "raw_payload_retained": False,
        "market_data_downloaded": False,
        "backtest_performed": False,
        "promotion_allowed": False,
        "requires_survivorship_audit_before_backtest": bool(spec["requires_survivorship_audit_before_backtest"]),
        "requires_liquidity_probe_before_backtest": bool(spec["requires_liquidity_probe_before_backtest"]),
        "quality_scope": "active_reference_seed_only_not_investable_universe",
    }


def validate_polygon_active_universe_seed_output(output_dir: str | Path = OUTPUT_DIR) -> dict[str, Any]:
    path = Path(output_dir)
    checks: list[dict[str, str]] = []
    required = ["preflight_report.json", "active_common_stock_seed.csv", "active_universe_seed_assessment.json", "final_decision.json"]
    ...
    assessment = json.loads((path / "active_universe_seed_assessment.json").read_text(encoding="utf-8"))
    sample = _read_csv(path / "active_common_stock_seed.csv")
    ...
    _check(checks, "survivorship_audit_still_required", decision.get("requires_survivorship_audit_before_backtest") is True, str(decision.get("requires_survivorship_audit_before_backtest")))
    _check(checks, "liquidity_probe_still_required", decision.get("requires_liquidity_probe_before_backtest") is True, str(decision.get("requires_liquidity_probe_before_backtest")))
    _check(checks, "active_seed_scope_only", assessment.get("quality_scope") == "active_reference_seed_only_not_investable_universe", str(assessment.get("quality_scope")))
...
def _metadata_coverage(rows: list[Any], required_fields: list[str]) -> dict[str, float]:
    ...
    total = len(rows)
    return {
        field: sum(1 for row in rows if isinstance(row, dict) and field in row and row.get(field) is not None) / total
...
def _empty_assessment(blockers: list[str]) -> dict[str, Any]:
    return {
        ...
        "provider": "Polygon/Massive",
        "record_count": 0,
        "active_common_stock_seed_count": 0,
        "metadata_coverage": {},
        "required_exchanges": ["XASE", "XNAS", "XNYS"],
        "required_security_type": "CS",
        "passes_active_universe_seed": False,
        "blockers": blockers,
        "active_common_stock_seed": [],
...
def _decision(assessment: dict[str, Any]) -> dict[str, Any]:
    passed = bool(assessment.get("passes_active_universe_seed"))
    return {
        "status": "complete",
        "decision": "POLYGON_ACTIVE_UNIVERSE_SEED_PASS" if passed else "POLYGON_ACTIVE_UNIVERSE_SEED_BLOCKED",
        "run_id": RUN_ID,
        "trial_id": TRIAL_ID,
        "record_count": assessment.get("record_count", 0),
        "active_common_stock_seed_count": assessment.get("active_common_stock_seed_count", 0),
        ...
        "requires_survivorship_audit_before_backtest": True,
        "requires_liquidity_probe_before_backtest": True,
        "next_unblocked_step": "Preregister survivorship/delisted audit and liquidity/price availability probes before any strategy backtest.",
    }
...
def _write_vault_report(path: Path, assessment: dict[str, Any], decision: dict[str, Any]) -> None:
    text = (
        "# Report Polygon Active Universe Seed 001 - 2026-05-24\n\n"
        f"Decision: `{decision['decision']}`\n\n"
        "## Scope\n\n"
        "Single bounded Polygon/Massive active ticker reference call. This creates a derived active common-stock seed only. No price download, backtest, parameter sweep, paper/live trading, short selling, or promotion occurred.\n\n"
        "## Result\n\n"
        f"- Records observed: {assessment['record_count']}\n"
        f"- Active common-stock seed count: {assessment['active_common_stock_seed_count']}\n"
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_ACTIVE_UNIVERSE_SEED_OUTPUT_PASS" if failed == 0 else "POLYGON_ACTIVE_UNIVERSE_SEED_OUTPUT_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Polygon active universe seed.")
    parser.add_argument("--validate-only", action="store_true")
    args = parser.parse_args(argv)
    if not args.validate_only:
        run_polygon_active_universe_seed_001()
    report = validate_polygon_active_universe_seed_output()
