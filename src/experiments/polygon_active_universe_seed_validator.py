...
REQUIRED_FILES = ["README.md", "seed_manifest.json", "blocked_actions.csv", "pass_fail_requirements.csv"]
...
REQUIRED_REQUIREMENTS = {
    "provider_call_count",
    "active_common_stock_seed_count",
    "metadata_coverage",
    "requires_survivorship_audit_before_backtest",
    "requires_liquidity_probe_before_backtest",
}


def validate_polygon_active_universe_seed_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    manifest = json.loads((path / "seed_manifest.json").read_text(encoding="utf-8"))
    ...
    exchanges = set(manifest.get("required_exchanges", []))

    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-ACTIVE-UNIVERSE-SEED-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-ACTIVE-SEED-001", str(manifest.get("trial_id")))
    ...
    _check(checks, "endpoint_fixed", manifest.get("endpoint") == "https://api.polygon.io/v3/reference/tickers", str(manifest.get("endpoint")))
    ...
    _check(checks, "active_reference_query", manifest.get("query_active") is True, str(manifest.get("query_active")))
    ...
    _check(checks, "common_stock_type", manifest.get("required_security_type") == "CS", str(manifest.get("required_security_type")))
    _check(checks, "candidate_threshold", int(manifest.get("min_active_common_stock_seed_count", 0)) >= 300, str(manifest.get("min_active_common_stock_seed_count")))
    _check(checks, "metadata_coverage_threshold", float(manifest.get("min_metadata_coverage", 0)) >= 0.95, str(manifest.get("min_metadata_coverage")))
    _check(checks, "survivorship_audit_required", manifest.get("requires_survivorship_audit_before_backtest") is True, str(manifest.get("requires_survivorship_audit_before_backtest")))
    _check(checks, "liquidity_probe_required", manifest.get("requires_liquidity_probe_before_backtest") is True, str(manifest.get("requires_liquidity_probe_before_backtest")))
    _check(checks, "required_fields_no_delisted_for_active_seed", fields == REQUIRED_FIELDS, f"fields={sorted(fields)}")
    _check(checks, "required_exchanges", REQUIRED_EXCHANGES.issubset(exchanges), f"missing={sorted(REQUIRED_EXCHANGES - exchanges)}")
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_ACTIVE_UNIVERSE_SEED_GATE_PASS" if failed == 0 else "POLYGON_ACTIVE_UNIVERSE_SEED_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon active universe seed gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_active_universe_seed_gate(args.spec_dir)
