...
def validate_polygon_ticker_details_listing_date_probe_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-TICKER-DETAILS-LISTING-DATE-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-LISTING-DATE-SOURCE-PROBE-001", str(manifest.get("trial_id")))
    _check(checks, "status_pre_run", manifest.get("status") == "APPROVED_SINGLE_TICKER_DETAILS_CALL_NOT_EXECUTED", str(manifest.get("status")))
    _check(checks, "provider_polygon", manifest.get("provider") == "Polygon/Massive", str(manifest.get("provider")))
    _check(checks, "ticker_fixed", manifest.get("ticker") == "A", str(manifest.get("ticker")))
    _check(checks, "endpoint_fixed", manifest.get("endpoint") == "https://api.polygon.io/v3/reference/tickers/A", str(manifest.get("endpoint")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_TICKER_DETAILS_LISTING_DATE_PROBE_GATE_PASS" if failed == 0 else "POLYGON_TICKER_DETAILS_LISTING_DATE_PROBE_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon ticker details listing-date probe gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_ticker_details_listing_date_probe_gate(args.spec_dir)
