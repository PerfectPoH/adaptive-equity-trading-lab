...
REQUIRED_PASS_FIELDS = {"ticker", "primary_exchange", "type", "active", "delisted_utc"}


def validate_polygon_ticker_reference_probe_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-TICKER-REFERENCE-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-SOURCE-PROBE-002", str(manifest.get("trial_id")))
    ...
    _check(checks, "limit_bounded", 0 < int(manifest.get("limit", 0)) <= 100, str(manifest.get("limit")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_TICKER_REFERENCE_PROBE_GATE_PASS" if failed == 0 else "POLYGON_TICKER_REFERENCE_PROBE_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon ticker reference probe gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_ticker_reference_probe_gate(args.spec_dir)
