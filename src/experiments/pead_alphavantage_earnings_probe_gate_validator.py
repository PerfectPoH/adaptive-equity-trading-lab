...
REQUIRED_FIELD_GROUPS = {"earnings_date", "actual_eps", "consensus_eps", "surprise", "timing", "pit_metadata"}
...
def validate_pead_alphavantage_earnings_probe_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-PEAD-ALPHAVANTAGE-EARNINGS-001", str(manifest.get("probe_id")))
    _check(checks, "status_pre_query", manifest.get("status") == "APPROVED_PRE_QUERY_NOT_EXECUTED", str(manifest.get("status")))
    _check(checks, "provider_alphavantage", manifest.get("provider") == "Alpha Vantage", str(manifest.get("provider")))
    _check(checks, "function_earnings", manifest.get("function") == "EARNINGS", str(manifest.get("function")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "PEAD_ALPHAVANTAGE_EARNINGS_PROBE_GATE_PASS" if failed == 0 else "PEAD_ALPHAVANTAGE_EARNINGS_PROBE_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = validate_pead_alphavantage_earnings_probe_gate(args.spec_dir)
