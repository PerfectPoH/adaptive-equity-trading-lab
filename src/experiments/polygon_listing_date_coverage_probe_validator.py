...
REQUIRED_FIELDS = {"ticker", "primary_exchange", "type", "active", "list_date"}


def validate_polygon_listing_date_coverage_probe_gate(spec_dir: str | Path) -> dict[str, Any]:
    path = Path(spec_dir)
    ...
    sample_source = Path(str(manifest.get("sample_source", "")))
    _check(checks, "probe_id", manifest.get("probe_id") == "PROBE-POLYGON-LISTING-DATE-COVERAGE-001", str(manifest.get("probe_id")))
    _check(checks, "trial_id", manifest.get("trial_id") == "UNIVERSE-LISTING-DATE-COVERAGE-PROBE-001", str(manifest.get("trial_id")))
    _check(checks, "status_pre_run", manifest.get("status") == "APPROVED_TEN_TICKER_DETAILS_CALLS_NOT_EXECUTED", str(manifest.get("status")))
    ...
    _check(checks, "sample_source_exists", sample_source.is_file(), str(sample_source))
    _check(checks, "sample_ticker_count", manifest.get("sample_ticker_count") == 10, str(manifest.get("sample_ticker_count")))
    _check(checks, "ten_call_bound", manifest.get("expected_max_calls") == 10, str(manifest.get("expected_max_calls")))
...
def _report(checks: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_decision": "POLYGON_LISTING_DATE_COVERAGE_PROBE_GATE_PASS" if failed == 0 else "POLYGON_LISTING_DATE_COVERAGE_PROBE_GATE_FAIL",
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Polygon listing-date coverage probe gate.")
    parser.add_argument("--spec-dir", required=True)
    args = parser.parse_args(argv)
    report = validate_polygon_listing_date_coverage_probe_gate(args.spec_dir)
